...
console.log(contarCaracteres("Viver é Demais"));
...
const converterCaracteresParaMinusculas = string => string.toLowerCase();
console.log(converterCaracteresParaMinusculas("CHOCOTONE E OVO DE PÁSCOA JUNTOS NO MERCADO EM PLENO FEVEREIRO"))
...
const posicaoDoCaractere = (caractere, string) => string.indexOf(caractere);
console.log(posicaoDoCaractere("i", "Boa Vida!"));
...
const itemExisteNoArray = (item, array) => array.includes(item);
console.log(itemExisteNoArray("Bola", ["Tenis", "Futebol", "Bola"]));
...
const concatenaArray = (array1, array2) => array1.concat(array2);
console.log(concatenaArray(["Bola", "Tenis"], ["Raquete", "Bolinha"]))
...
const arraySemUltimoItem = array => array.slice(0 ,array.length -1);
console.log(arraySemUltimoItem(["Bola", "Tenis", "Raquete", "Bolinha"]))
...
const valorNulo = value => value === null
...
const invocarCallback = callback => callback(); 
const exibirNome = name => console.log(`Meu Nome é ${name}!`); 
invocarCallback(exibirNome("Ronald"));
...
// Função que invoca uma função de callback com um valor
function invocarCallbackComValor(callback, valor) {
...
// Chamando a função para obter o triplo de 33
const resultado = invocarCallbackComValor(triplo, 33);

console.log(resultado); // Isso irá exibir 99 no console
