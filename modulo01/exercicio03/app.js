...
const initials = `${dogNames[0].charAt(0)}${dogNames[1].charAt(0)}${dogNames[2].charAt(0)}${dogNames[3].charAt(0)}`
...
parte "ca" do 2º item e a parte "naque não deve ser  
digitada diretamente;" do 4º item;
- A string deve resultar na palavra 'cocana', 
...
const dessert = `${dogNames[0].charAt(dogNames[0].length-2)}${dogNames[0].charAt(dogNames[0].length-1)}${dogNames[1].charAt(dogNames[1].length-2)}${dogNames[1].charAt(dogNames[1].length-1)}${dogNames[3].charAt(dogNames[3].length-2)}${dogNames[3].charAt(dogNames[3].length-1)}`;

//console.log(dessert.replace("n", "d"))

const dessertFinal = `A ${dessert.replace("n", "d")} é um doce à base de coco, tradicional na América Latina e em Angola. `
